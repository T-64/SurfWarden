...
- ⏱ **准确计时**：active tab + 系统 idle/锁屏检测；MV3 后台休眠不虚增（心跳水位 + 宽限截断，ADR-0003）
- 🧭 **内容级规则**：域名 / 路径通配 / 完整 URL / 正则，更具体的规则永远赢——`live.bilibili.com → 娱乐` 与 `B站德语课 → 学习` 可以共存
- 🎯 **预算监督**：类别预算 → 80% 横幅提醒 → 超额重定向到引导页（主按钮"去学习"一键跳回你的课程）
- 🪙 **豁免机制**：引导页"再来 5 分钟"，每天限次，如实计入数据
- 🕶 **隐身覆盖**：检测 + 图文引导开启隐身授权，隐身浏览照常计入（不给监督留后门）
- 📊 **Popup**：环形图、预算余量、Top 站点、当前页一键改判（"本次记为学习"）、暂停监督 30 分钟
